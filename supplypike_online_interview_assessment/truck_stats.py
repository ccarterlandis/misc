...
class TruckStatsFinder():
	...
	def find_count_of_active_trucks(self):
		...
		#this is the part that could use some work to make it quicker by only checking years in valid range and being smarter about knowing what years to check
	...
	def find_years_with_config_minmax_active_trucks(self, num_of_min_trucks, num_of_max_trucks):
		#copy_of_count_trucks is for manipulation. since I pop the keys when I find them in order to prevent duplicates, when actually printing them out I need to be able to reference them.
		copy_of_count_trucks = self.count_trucks_by_year.copy()
		self.num_of_min_trucks = num_of_min_trucks
		self.num_of_max_trucks = num_of_max_trucks

		for n in range(0, num_of_min_trucks):
			year_with_config_min_trucks = min(copy_of_count_trucks, key=copy_of_count_trucks.get)

			if year_with_config_min_trucks not in config_count_of_min_trucks:
				config_count_of_min_trucks.append(year_with_config_min_trucks)
				copy_of_count_trucks.pop(year_with_config_min_trucks, None)

		for n in range(0, num_of_max_trucks):
			year_with_config_max_trucks = max(copy_of_count_trucks, key=copy_of_count_trucks.get)

			if year_with_config_max_trucks not in self.config_max_trucks:
				self.config_max_trucks.append(year_with_config_max_trucks)
				copy_of_count_trucks.pop(year_with_config_max_trucks, None)
	...
	def print_stats(self):
		...
		print("Least-{}: {}".format(self.num_of_min_trucks, config_count_of_min_trucks))
		print("Top-{}: {}".format(self.num_of_max_trucks, config_count_of_min_trucks))
